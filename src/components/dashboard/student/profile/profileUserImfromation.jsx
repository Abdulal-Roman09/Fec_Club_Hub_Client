...
import { Mail, Phone, MapPin, User, Briefcase, Calendar } from "lucide-react";

const ProfileUserInformation = ({ user, userProfile }) => {
  if (!user || !userProfile) return null;

  return (
    <div className="bg-green-50 p-4 sm:p-6 md:p-8 rounded-2xl shadow-md">
      <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8">
        {/* Avatar - keep big */}
        <div className="w-48 h-48 md:w-72 md:h-72 bg-grreen-50 rounded-full flex items-center justify-center text-white text-3xl md:text-5xl font-bold shadow-md overflow-hidden">
          {user.photoURL ? (
            <img
              src={user.photoURL}
        ...
        <div className="md:text-left flex-1 space-y-2">
          ...
          <p className="text-sm sm:text-base text-gray-700 flex items-center gap-1">
            <Mail size={16} /> <span className="font-bold">Email:</span>{" "}
            {user.email}
          </p>

          {userProfile.phone && (
            <p className="text-sm sm:text-base text-gray-700 flex items-center gap-1">
              <Phone size={16} /> <span className="font-bold">Phone:</span>{" "}
              {userProfile.phone}
            </p>
          )}
          <p className="text-sm sm:text-base md:text-lg font-medium flex items-center gap-1">
            <Briefcase size={16} />
          ...
          {userProfile.address && (
            <div className="text-sm sm:text-base text-gray-700 flex flex-col gap-1">
              ...
              <p className="ml-5">
                <span className="font-bold">Zela:</span>{" "}
                {userProfile.address.zela || ""}
              </p>
              <p className="ml-5">
                <span className="font-bold">UpZila:</span>{" "}
