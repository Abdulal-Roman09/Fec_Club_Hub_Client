    ...
    <div className="hidden md:flex items-center gap-4">
      ...
      {user ? (
          ...
          {openModal && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
              <div className="flex flex-col">
                <Link to="/dashboard/profile" onClick={() => setOpenModal(false)} className="px-4 py-2 hover:bg-gray-100 transition">Profile</Link>
                <Link to="/dashboard" onClick={() => setOpenModal(false)} className="px-4 py-2 hover:bg-gray-100 transition">Dashboard</Link>
                <button onClick={handleLogout} className="px-4 py-2 text-left hover:bg-gray-100 transition">Logout</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2">
          <Link to="/auth/login" className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-medium transition">Login</Link>
          <Link to="/auth/register" className="bg-gray-200 hover:bg-gray-300 px-4 py-2 rounded-lg font-medium transition">Register</Link>
        </div>
      )}
    </div>
  );
